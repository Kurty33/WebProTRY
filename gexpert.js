...
function makeAIMove() {
    ...
    if (emptyBoxes.length > 0) {
        // Check for blocking human's winning moves
        ...
        if (blockingMove) {
            ...
            // Make a blocking move if no winning moves
            if (!winningMove) {
                const randomIndex = Math.floor(Math.random() * emptyBoxes.length);
                winningMove = emptyBoxes[randomIndex];
                winningMove.innerHTML = 'O';
            }
        }
 
        // Call Win(), Draw(), and Turns() after all checks are complete
        Win();
        Draw();
        Turns();
    }
